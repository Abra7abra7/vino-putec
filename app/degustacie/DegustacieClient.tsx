...
export default function DegustacieClient() {
    ...
    <div className="min-h-screen bg-background">
      ...
      <section className="py-16 bg-accent-light">
        <div className="container mx-auto px-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-2 gap-8">
            {packages.map((pkg) => (
              <div key={pkg.id} className="bg-background rounded-lg shadow-lg p-8 border border-accent flex flex-col h-full">
                {/* Package Header */}
                <div className="text-center mb-6">
                  <div className="text-4xl mb-4">{pkg.emoji}</div>
                  <h3 className="text-2xl font-bold text-foreground mb-2">{pkg.title}</h3>
                  <h4 className="text-xl text-foreground font-semibold mb-4">{pkg.subtitle}</h4>
                ...
                <div className="flex flex-col sm:flex-row gap-3 mt-auto">
                  <button
                    ...
                    className="flex-1 bg-accent hover:bg-accent-dark text-foreground px-6 py-3 rounded-lg font-semibold transition-colors"
                  >
                    Rezervovať termín
                  </button>
                  <button
                    onClick={() => handlePayment(pkg.id)}
                    className="flex-1 border-2 border-accent text-foreground hover:bg-accent hover:text-foreground px-6 py-3 rounded-lg font-semibold transition-colors"
                  >
                    Zaplatiť teraz
                  </button>
                </div>
      ...
      {showBookingForm && selectedPackage && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
          <div className="bg-background rounded-xl shadow-2xl max-w-lg w-full border-2 border-accent">
            {/* Header */}
            <div className="p-4 border-b border-accent">
              <div className="flex justify-between items-center mb-3">
                ...
                <button
                  ...
                  className="text-foreground hover:text-foreground-dark text-xl font-bold bg-accent-light hover:bg-accent rounded-full w-7 h-7 flex items-center justify-center transition-colors"
                >
                  ×
                </button>
              </div>
              
              <div className="p-2 bg-accent-light rounded border border-accent">
                <p className="text-foreground font-bold text-center text-sm">
              ...
              <form className="space-y-3">
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                  <div>
                    ...
                    <input
                      type="text"
                      className="w-full border-2 border-accent p-2 rounded focus:outline-none focus:ring-1 focus:ring-accent text-foreground bg-background text-sm"
                ...
                <div>
                  ...
                  <select className="w-full border-2 border-accent p-2 rounded focus:outline-none focus:ring-1 focus:ring-accent text-foreground bg-background text-sm">
                ...
                <div>
                  ...
                  <textarea
                    rows={2}
                    className="w-full border-2 border-accent p-2 rounded focus:outline-none focus:ring-1 focus:ring-accent text-foreground bg-background resize-none text-sm"
            ...
            <div className="p-4 border-t border-accent">
              <div className="flex space-x-3">
                <button
                  type="button"
                  onClick={() => setShowBookingForm(false)}
                  className="flex-1 border-2 border-accent text-foreground hover:bg-accent hover:text-foreground px-4 py-2 rounded font-semibold transition-colors text-sm"
                >
                  Zrušiť
                </button>
                <button
                  type="submit"
                  className="flex-1 bg-accent hover:bg-accent-dark text-foreground px-4 py-2 rounded font-semibold transition-colors text-sm"
                >
                  Odoslať rezerváciu
                </button>
              </div>
